...
requirePermission('edit_finance_tasks');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE finance_tasks SET action_req_by = ?, request_date = ?, cost_center = ?, action_req = ?, action_owner = ?, status = ?, completion_date = ?, remark = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    
    if ($stmt) {
        $stmt->bind_param("ssssssssi", $action_req_by, $request_date, $cost_center, $action_req, $action_owner, $status, $completion_date, $remark, $id);
        
        if ($stmt->execute()) {
            $success = "Task updated successfully!";
            $auth->logAction('update_finance_task', 'finance_tasks', $id);
            // Refresh task data
            $sql = "SELECT * FROM finance_tasks WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("i", $id);
...
<head>
    ...
    <title>Edit Task - Tracker Updates</title>
                ...
                <div class="mb-8">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-gray-600 mt-2">Update task #<?= $task['id'] ?> - <?= htmlspecialchars($task['action_req_by']) ?></p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" id="action_req_by" name="action_req_by" required
                                       value="<?= htmlspecialchars($task['action_req_by']) ?>"
                            ...
                            <div>
                                ...
                                <input type="date" id="request_date" name="request_date" required
                                       value="<?= htmlspecialchars($task['request_date']) ?>"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="cost_center" name="cost_center" required
                                       value="<?= htmlspecialchars($task['cost_center']) ?>"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="action_owner" name="action_owner" required
                                       value="<?= htmlspecialchars($task['action_owner']) ?>"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" id="completion_date" name="completion_date"
                                       value="<?= htmlspecialchars($task['completion_date']) ?>"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="action_req" name="action_req" rows="4" required
                                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"><?= htmlspecialchars($task['action_req']) ?></textarea>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="remark" name="remark" rows="3"
                                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"><?= htmlspecialchars($task['remark']) ?></textarea>
                            </div>
                        </div>
