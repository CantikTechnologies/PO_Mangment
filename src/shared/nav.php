...
<header class="flex items-center justify-between whitespace-nowrap border-b border-solid border-gray-200 bg-white px-10 py-3">
  <div class="flex items-center gap-3 text-gray-900">
    <a href="/PO-Management/public/index.php" aria-label="Cantik Homemade" class="inline-flex items-center justify-center">
      <span class="inline-flex p-[2px] rounded-full bg-gradient-to-br from-rose-500 via-fuchsia-500 to-indigo-500 shadow-sm ring-1 ring-rose-200/50">
        <span class="inline-flex items-center justify-center h-9 w-9 rounded-full bg-white">
          <img src="/PO-Management/public/assets/cantik_logo.png" alt="Cantik Homemade" class="h-7 w-7 object-contain"/>
        </span>
      </span>
    </a>
  ...
  <div class="flex flex-1 items-center justify-end gap-4">
    <nav class="hidden md:flex items-center gap-2">
      <?php $dashActive = $activeClass(["/PO-Management/public/index.php", "/PO-Management/"]); ?>
      <a class="rounded-full px-4 py-2 text-sm font-medium <?= $dashActive ? 'text-rose-600 bg-rose-50' : 'text-gray-700 hover:bg-gray-100' ?>" href="/PO-Management/public/index.php">Dashboard</a>
      
      <?php $poActive = $activeClass(["/PO-Management/src/po_details/"]); ?>
      <div class="relative group">
        <a class="rounded-full px-4 py-2 text-sm font-medium <?= $poActive ? 'text-rose-600 bg-rose-50' : 'text-gray-700 hover:bg-gray-100' ?>" href="/PO-Management/src/Modules/po_details/list.php">
          Purchase Orders
        ...
        <div class="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
          <a href="/PO-Management/src/Modules/po_details/list.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-t-lg">View All POs</a>
          <?php if (hasPermission('add_po_details')): ?>
          <a href="/PO-Management/src/Modules/po_details/add.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Add New PO</a>
          <?php endif; ?>
        </div>
      </div>
      
      <?php $invActive = $activeClass(["/PO-Management/src/invoices/"]); ?>
      <div class="relative group">
        <a class="rounded-full px-4 py-2 text-sm font-medium <?= $invActive ? 'text-rose-600 bg-rose-50' : 'text-gray-700 hover:bg-gray-100' ?>" href="/PO-Management/src/Modules/invoices/list.php">
          Invoices
          <span class="material-symbols-outlined text-sm ml-1">expand_more</span>
        </a>
        <div class="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
          <a href="/PO-Management/src/Modules/invoices/list.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-t-lg">View All Invoices</a>
          <?php if (hasPermission('add_invoices')): ?>
          <a href="/PO-Management/src/Modules/invoices/add.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Add New Invoice</a>
          <?php endif; ?>
        </div>
      </div>
      
      <?php $outActive = $activeClass(["/PO-Management/src/outsourcing/"]); ?>
      <div class="relative group">
        <a class="rounded-full px-4 py-2 text-sm font-medium <?= $outActive ? 'text-rose-600 bg-rose-50' : 'text-gray-700 hover:bg-gray-100' ?>" href="/PO-Management/src/Modules/outsourcing/list.php">
          Outsourcing
          <span class="material-symbols-outlined text-sm ml-1">expand_more</span>
        </a>
        <div class="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
          <a href="/PO-Management/src/Modules/outsourcing/list.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-t-lg">View All Records</a>
          <?php if (hasPermission('add_outsourcing')): ?>
          <a href="/PO-Management/src/Modules/outsourcing/add.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Add New Record</a>
          <?php endif; ?>
        </div>
      </div>
      
      <?php $soActive = $activeClass(["/PO-Management/src/so_form.php"]); ?>
      <a class="rounded-full px-4 py-2 text-sm font-medium <?= $soActive ? 'text-rose-600 bg-rose-50' : 'text-gray-700 hover:bg-gray-100' ?>" href="/PO-Management/public/so_form.php">Reports</a>
      
      <?php $trackerActive = $activeClass(["/PO-Management/src/Tracker%20Updates/"]); ?>
      <div class="relative group">
          <a class="rounded-full px-4 py-2 text-sm font-medium <?= $trackerActive ? 'text-rose-600 bg-rose-50' : 'text-gray-700 hover:bg-gray-100' ?>" href="/PO-Management/src/Modules/Tracker/index.php">
          Tracker Updates
          <span class="material-symbols-outlined text-sm ml-1">expand_more</span>
        </a>
        <div class="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
          <a href="/PO-Management/src/Modules/Tracker/index.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-t-lg">View All Tasks</a>
          <a href="/PO-Management/src/Modules/Tracker/add.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Add New Task</a>
        </div>
      </div>
    </nav>
